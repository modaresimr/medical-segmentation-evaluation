...
class MME(Metric):
    ...
    def precompute(self, reference, spacing=None, connectivitiy=1):
        ...
        for c in range(self.num_classes):
          print(c)
          with Timer.instance(f'class {c}'):
            refc = reference == c
            with Timer.instance(f'connected_components'):
            ...
            with Timer.instance(f'region watershed'):
                # if c==0:
            ...
            for i in range(0, gN + 1):
              with Timer.instance(f'component{i}'):
