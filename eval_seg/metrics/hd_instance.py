    ...
    def evaluate(self, test: np.ndarray, return_debug_data:bool=False,**kwargs):
        ...
        for c in range(self.num_classes):
            ...
            for ri in range(1, dc.gN + 1):
                ...
                dci.pred_border_dst = geometry.distance(dci.component_pred, mode='both', mask=dci.rel_gt_comps | dci.component_pred, spacing=self.spacing)
                ...
                dci.pred_hd = dci.dst_border_pred2gt_abs.max() if valid else np.nan

                dci.pred_hd_avg = dci.dst_border_pred2gt_abs.mean() if valid else np.nan

                dci.pred_hd95 = np.quantile(dci.dst_border_pred2gt_abs, 0.95) if valid else np.nan
