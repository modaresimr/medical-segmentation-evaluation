...
class Voxel(np.ndarray):
    def __new__(cls,ndarray):
        if ndarray.ndim!=3:raise Exception('Only 3d is supported')
    ...
    def __getstate__(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.avi') as temp:
            ...
            vw = cv2.VideoWriter(temp.name,p, codec, 1,(self.shape[0],self.shape[1]), False)
    ...
    def __setstate__(self, d):
        ...
        with tempfile.NamedTemporaryFile() as temp:
            ...
            for i in len(allf):
                res[:,:,i]=(allf[i][:,:,0]<<16)|(allf[i][:,:,1]<<8)|allf[i][:,:,2]
...
def convert2png_array(img3d):
        ...
        for z in range(0,data.shape[2]):
            img = data[:,:,z]
            bstr.append(cv2.imencode('.png',img.astype(np.uint16),[int(cv2.IMWRITE_PNG_COMPRESSION), 1])[1].tobytes())
        return bstr
