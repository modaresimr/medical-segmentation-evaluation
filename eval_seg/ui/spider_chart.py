...
def spider_chart(df, rng=[0, .5, 1], title=None, **kwargs):
    # N = len(list(df))
...
def _spider_chart(df, rng, title, ax, theta):
    ...
    ax.set_title(title, position=(0.5, 1.2), horizontalalignment='center', verticalalignment='center')
    ...
    ax.margins(x=0, y=0)
    legend = ax.legend(loc=(0.8, .9), labelspacing=0.1, fontsize='small')
...
def spider_chart_multi(dic, rng, title=None, cols=5, **kwargs):
    ...
    fig, axes = plt.subplots(rows, cols, figsize=(4 * cols, 4 * rows), subplot_kw=dict(projection=axis_class))
