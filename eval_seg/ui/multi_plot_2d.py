...
def multi_plot_2d(img, pred, dst=None, spacing=None, args={}):
    ...
    if args.get('zoom2segments', 0):
        ...
        zoom_roi = ct.segment_roi([items[p] for p in items if p != imglbl], wh_ratio=orig_ratio, mindim=[20 / spacing[0], 20 / spacing[1], -1])
    ...
    for anim in range(zs):
        ...
        for i, p in enumerate(data):
            ...
            if p in [imglbl, origsize_lbl]:
                axes[i].imshow(current['pred'], cmap=mri_cmap, vmin=0, vmax=1, alpha=1, interpolation='nearest', aspect=aspect)
                if p == origsize_lbl:
                ...
                if args.get('add_backimg', 0) and not ():
                    axes[i].imshow(normalimg[:, :, anim] / 2, cmap=mri_cmap, vmin=0, vmax=1, alpha=1, interpolation='nearest', aspect=aspect)
                if p != gtlbl:
                    if args.get('show_tp_fp_fn', 1):
                        if 'tp' in current and current['tp'].sum() > 0:
                            ...
                            axes[i].imshow(current['tp'], cmap=cmap, vmin=0, vmax=1, alpha=1, aspect=aspect)
                        if 'fp' in current and current['fp'].sum() > 0:
                            cmap = ListedColormap([(0, 0, 0, 0), 'yellow'])
                            axes[i].imshow(current['fp'], cmap=cmap, vmin=0, vmax=1, alpha=1, aspect=aspect)
                        ...
                        if 'fn' in current and current['fn'].sum() > 0:
                            ...
                            axes[i].imshow(current['fn'], cmap=cmap, vmin=0, vmax=1, alpha=1, aspect=aspect)
                            axes[i].contour(current['fn'], corner_mask=False, cmap=cmap, vmin=0, vmax=1)
                    else:
                        cmap = ListedColormap([(0, 0, 0, 0), 'lime'])
                        axes[i].imshow(data[gtlbl]['pred'], cmap=cmap, vmin=0, vmax=1, alpha=.5, aspect=aspect)
