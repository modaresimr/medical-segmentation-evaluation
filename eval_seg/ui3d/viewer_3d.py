...
def multi_plot(gt, preds, spacing=None,args={}):
    ...
    preds={pr:preds[pr].reshape(preds[pr].shape[0],preds[pr].shape[1],-1) for pr in preds}
    
    hasz=gt.shape[2]>2
    
    
    
    plot = k3d.plot(grid=[0, gt.shape[0], 0, gt.shape[1], 0, gt.shape[2]], name=args['dst'], grid_auto_fit=False,camera_auto_fit=hasz)
    if args['show']:
        plot.display()
    if not hasz:
        plot.camera = [512, 200, 200, 0, 200, 200, 0, 0, 1]

    v= k3d.voxels(gt.astype(np.uint8), opacity=0.3, compression_level=9, name='gt', group='gt',scaling=spacing)
    ...
    #         skeleton = skeletonize(shape)
            ...
            v2= k3d.voxels(skeleton.astype(np.uint8), opacity=.2, color_map=[cls_colormap[c]], compression_level=1, name=f'smooth-{c}', group='gt',scaling=spacing)
    ...
    for p in tqdm(preds,leave=False):
        ...
        v=k3d.voxels(fp.astype(np.uint8), opacity=0.6, color_map=fp_colormap, compression_level=9, name='fp', group=p,scaling=spacing)
        v.visible=args.get('show_all_fp',0)
        v.outlines=False
        plot += v
        
        v= k3d.voxels(fn.astype(np.uint8), opacity=0.6, color_map=fn_colormap, compression_level=9, name='fn', group=p,scaling=spacing) #
        v.visible=args.get('show_all_fn',0)
        v.outlines=False
        plot += v
        
        
        v= k3d.voxels(tp.astype(np.uint8), opacity=0.2, color_map=tp_colormap, compression_level=9, name='tp', group=p,scaling=spacing)
        v.visible=args.get('show_all_tp',0)
        v.outlines=False
        plot +=v
        
        v= k3d.voxels(pred.astype(np.uint8) , opacity=0.2, compression_level=9, color_map=cls_colormap, name='pred', group=p,scaling=spacing)
        v.visible=args.get('show_all_pred',0)
        v.outlines=False
        plot +=v
        
        for c in tqdm(range(1, int(pred.max() + 1)),leave=False):
            v = k3d.voxels((pred == c).astype(np.uint8), opacity=0.2, color_map=[cls_colormap[c]], compression_level=9, name=f'{c}', group=p,scaling=spacing)
